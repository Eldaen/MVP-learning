...
protocol ImageCache: AnyObject {
	...
	func getImage(for url: URLConvertible) -> UIImage?
	
	/// Cохраняет изображение по URL
	func saveImage(_ image: UIImage?, for url: URLConvertible)
	
	/// Удаляет изображение из кэша
	func deleteImage(for url: URLConvertible)
	
	/// Чистит кэш
	func сlearCache()
	
	// Пусть будет сабскрипт для доступа, хочется попробовать
	subscript(_ url: URLConvertible) -> UIImage? { get set }
}
...
extension ImageCacheService: ImageCache {
	...
	func getImage(for url: URLConvertible) -> UIImage? {
		if let image = imageCache.object(forKey: url as AnyObject) as? UIImage {
			return image
		} else if let urlString = try? url.asURL().absoluteString,
				  let image = loadImageFromDiskWith(imageName: urlString){
			return image
		} else {
			return nil
		}
	...
	func saveImage(_ image: UIImage?, for url: URLConvertible) {
		...
		let urlString = try? url.asURL().absoluteString else {
		...
		DispatchQueue.global(qos: .background).async { [weak self] in
			self?.saveImage(imageName: urlString, image: image)
		}
	}
	...
	subscript(_ url: URLConvertible) -> UIImage? {
		get {
